...
def fazer_pedido(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            pedido = form.save()
            #get Comanda
            comanda = comanda_models.models.Comanda.objects.get(user=request.POST.get('user'))
            #Add Cota
            comanda_models.Cota.objects.create(pedido=pedido.pk,comanda=comanda.pk)
...
def lista_pedidos(request):
    ...
    for p in pedidos:
        ...
        cota = comanda_models.Cota.objects.get(pedido=p.pedido.pk).count()
        custo = round((atual.quantidade*atual.produto.preco)/cota,2)
        context.update({atual.pk:{"Custo": custo,"Quantidade":atual.quantidade,
            "Estado":atual.estado,"Nome":atual.produto.nome,"Foto_Produto":json.dumps(str(atual.produto.foto))}})
